public class Constants {
    public static float MONSTER_LVL_EXP_RATIO = 2F;
    public static int MONSTER_LVL_GOLD_RATIO = 100;
    public static float HERO_REVIVE_RATIO = 0.5F;
    public static int HERO_EXP_PER_LVL = 10;
    public static float TRADEIN_COEFFICIENT = 0.5F;

    public static int DEFAULT_INVENTORY_ACTIVE_SIZE = 3;
    public static float SPELL_EFFECT_COEFFICIENT = 0.1F;

    public static int HERO_LEVELUP_HP_RATIO = 100;
    public static float HERO_LEVELUP_MANA_RATIO = 1.1F;

    public static int HERO_PARTY_SIZE_MIN = 3;
    public static int HERO_PARTY_SIZE_MAX = 3;

    public static float ATTR_BUFF_RATIO = 1.5F;

    public static int[] VALOR_MONSTER_LANE = new int[]{0,3,6};
}
